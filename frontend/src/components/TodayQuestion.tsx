...
const TodayQuestion = ({
    ...
    const loadData = async () => {
      try {
        if (state.selectedAcount) {
          ...
          if (wonBy && wonBy !== ZeroAddress) {
            setWinner(wonBy);
            const submittedAnswer = await fetchQuestionAnswer(wonBy)
  ...
  const onSubmitAnswer = async () => {
    try {
      console.log(`submit answer to snap`, answer, referralAddress, state.selectedAcount);
      setLoading(true);
      ...
      console.log('state chain is: ', state.chain)
